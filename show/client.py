...
class ShowClient():
    '''Client which fetches the show informations from the cast-web-service
       ...
       See http://intranet.rabe.ch/mantisbt/view.php?id=20 for more informations
    '''
    ...
    def update(self):
        ...
        # goes wrong later.
        self.show.set_endtime(datetime.datetime.now(pytz.timezone('UTC')) + \
        ...
        if len(real_name) == 0 or real_name[0].hasChildNodes() == False:
        ...
        if len(end_time) == 0 or end_time[0].hasChildNodes() == False:
        ...
        if len(start_time) == 0 or start_time[0].hasChildNodes() == False:
        ...
        if self.show.endtime < datetime.datetime.now(pytz.timezone('UTC')):
            logger.error('%s: Show endtime %s is in the past' % \
                            (self.__class__, self.show.endtime))

            raise ShowClientError('Show end time (%s) is in the past' % \
                                   self.show.endtime)
        ...
        if len(url) == 0 or start_time[0].hasChildNodes() == False:
            logger.error('%s: No <url> tag found' % self.__class__)
        else:
            self.show.set_url(url[0].firstChild.data.strip())


        logger.info('Show "%s" started and runs from %s till %s' %
